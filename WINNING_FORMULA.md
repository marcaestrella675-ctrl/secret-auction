...
```env
# Sepolia 测试网配置
PRIVATE_KEY=0xd14a24a2946c0e528036a470ad0bd6597ef2d98674a48072a845e5f7a75ee22d
...
#### 错误 9.5: Vercel 构建失败 - TypeScript 类型错误

**错误信息**:
```
Type error: Argument of type '{ appName: string; projectId: string; chains: ...; ssr: boolean; autoConnect: boolean; connectors: ... }' 
is not assignable to parameter of type 'GetDefaultConfigParameters'.
  Object literal may only specify known properties, and 'autoConnect' does not exist in type 'GetDefaultConfigParameters'.
  Object literal may only specify known properties, and 'connectors' does not exist in type 'GetDefaultConfigParameters'.
```

**出现场景**:
- 部署到 Vercel 时构建失败
- 本地 `pnpm build` 报 TypeScript 类型错误

**根本原因**:
`getDefaultConfig` 函数只接受 4 个核心参数，不再支持 `autoConnect` 和 `connectors` 参数。自动连接功能由库内部管理，默认连接器集合（MetaMask、WalletConnect 等）会自动提供。
...
- [ ] 移除 `autoConnect` 参数
- [ ] 移除 `connectors` 参数
- [ ] 只保留 4 个核心参数：`appName`, `projectId`, `chains`, `ssr`
- [ ] 本地运行 `pnpm build` 测试
- [ ] 确认版本：`@rainbow-me/rainbowkit@^2.0.0`, `wagmi@^2.0.0`

**参考文档**:
- RainbowKit 迁移指南: https://www.rainbowkit.com/docs/migration-guide
...
- **RainbowKit v2 迁移指南**: https://www.rainbowkit.com/docs/migration-guide
...
packages/nextjs-showcase/components/Providers.tsx   # RainbowKit 配置
...
**版本**: V4.2 (Pure FHEVM v0.9 + RainbowKit v2+)  
**更新日期**: 2025-11-30  
**说明**: 简化 RainbowKit v2+ 配置说明，移除新老版本对比，只保留最新实践
